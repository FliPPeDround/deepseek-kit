...
`prompt` and `messages` are mutually exclusive input methods. `prompt` is suitable for single requests, while `messages` is for multi-turn conversations. `system` can be combined with either.
::
