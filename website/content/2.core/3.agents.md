...
- **`threshold`** (number, default 0.85) — Context usage ratio that triggers compaction. When `prompt_tokens >= contextWindowSize * threshold`, history is summarized.
- **`keepRecentRounds`** (number, default 3) — Number of recent conversation rounds to preserve without summarization.
- **`model`** (Model, default `'deepseek-v4-flash'`) — The LLM used to generate summaries.
- **`contextWindowSize`** (number, default 1,000,000) — The model's context window size in tokens.

### How It Works

1. At the start of each step, the agent checks if the previous step's `prompt_tokens` exceeded the threshold
2. If triggered, older conversation rounds are summarized into a single message via LLM
3. System prompts, few-shot examples, and the most recent rounds are always preserved
4. The summary is inserted as a `user` message with `name: 'compact-summary'`
...
::field-group
  ...
  ::field{name="compact" type="boolean | AgentCompactConfig"}
  Enable context compaction. When `true`, older conversation rounds are automatically summarized when context usage approaches the limit. Pass an object `{ threshold?: number, keepRecentRounds?: number, model?: Model, contextWindowSize?: number }` for custom settings.
  ::
