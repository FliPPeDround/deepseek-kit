...
启用严格模式后，deepseek-kit 会在 JSON Schema 层面强制约束参数结构——设置 `additionalProperties: false` 并将所有属性标记为必需。这能提高模型生成参数的可靠性：
...
严格模式会修改生成的 JSON Schema，确保模型不会生成 Schema 之外的字段。但并非所有模型都支持严格模式，不支持的模型会忽略此选项。
::
...
::field-group
  ...
  启用严格模式。强制 JSON Schema 设置 `additionalProperties: false` 并将所有属性标记为必需。
  ::
